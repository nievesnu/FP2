...
bool procesarEntradas( tListaPelis& listaP, tListaPeticiones& listaEn, int suma)
{
	bool procesadas = false;
	
	for (int i = 0; i < listaP.numPelis; i++)
	{
		for (int j = 0; j < listaEn.capacidad; j++)
		{
			while (listaP.arrayPelis[i]->titulo == listaEn.arrayEntradas[j].titulo)
			{
				if (listaP.arrayPelis[i]->num_disponibles <= listaEn.arrayEntradas[j].num_solicitadas)
				{
					suma = listaP.arrayPelis[i]->precio * listaEn.arrayEntradas[j].num_solicitadas;
					listaP.arrayPelis[i]->num_disponibles = listaP.arrayPelis[i]->num_disponibles - listaEn.arrayEntradas[j].num_solicitadas;
					procesadas = true;
				}
				else
				{
					suma = listaP.arrayPelis[i]->precio * listaP.arrayPelis[i]->num_disponibles;
					listaP.arrayPelis[i]->num_disponibles = 0;
					procesadas = true;
				}
			}
		}if (listaP.arrayPelis[i]->titulo != listaEn.arrayEntradas[i].titulo)
		{
			cout << "Esta pelicula no existe en este cine" << endl;
			procesadas = false;
		}
...
int costeEntradasSolicitadas()
{
	return 0;
}

void liberarMem(tListaPelis& lista)
{
	for (int i = 0; i < MAX_Pelis; i++)
	{
		delete lista.arrayPelis[i];
		lista.arrayPelis[i] = nullptr;
	}
	lista.numPelis = 0;
}
